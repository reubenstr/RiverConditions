...
bool PopulateStationInitFromSDCard()
{
  ...
  if (!GetJsonFromSDCard("stations", &stationInitDataJson))
  {
    return false;
  }
  ...
  {
    stations[i].id = doc["stations"][i]["id"].as<String>();
    stations[i].shortName = doc["stations"][i]["shortName"].as<String>();
    stations[i].location = doc["stations"][i]["location"].as<String>();
  }
...
bool UpdateStationDataOnScreen(int stationIndex, String *stationDataJson)
{
  ...
  tft.setTextColor(ILI9341_WHITE, ILI9341_BLACK);
  tft.setCursor(5, 5);
  tft.printf("%-17s", stations[stationIndex].shortName.c_str());
  tft.setCursor(5, 30);
  tft.printf("%-17s", stations[stationIndex].location.c_str());

  tft.setTextSize(2);
  tft.setTextColor(ILI9341_GREEN, ILI9341_BLACK);

  DynamicJsonDocument doc(2048);
  DeserializationError error = deserializeJson(doc, *stationDataJson);
...
void loop(void)
{
  ...
  if (!GetJsonFromSDCard(stations[stationIndex].id, &stationDataJson))
  {
    //FatalError("Failed to get station " + stations[stationIndex].id + "'s data.");
    UpdateStationDataOnScreen(stationIndex, &stationDataJson);
  }
  else
  {
    UpdateStationDataOnScreen(stationIndex, &stationDataJson);
